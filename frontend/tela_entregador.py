...
class Tela_entregador(object):
    """
         Classe responsável por definir a interface gráfica relacionada à gestão de produtos.

        Attributes:
        ----------
        PAGINAS (QtWidgets.QStackedWidget): Widget de pilha para gerenciar diferentes páginas da interface.

        Methods:
        --------
        setupUi(self, tela_usuario): Configuração da interface do usuário.
        retranslateUi(self, tela_usuario): Tradução de elementos da interface.
        carregar_dados(self): Carrega dados relacionados aos produtos na interface.
        exibir_pagina(self, indice): Exibe a página correspondente ao índice na pilha de páginas.
    ...
    def retranslateUi(self, tela_usuario):
        ...
        """
            Tradução de elementos da interface.

            Parameters:
            ----------
            tela_usuario (QtWidgets.QMainWindow): Janela principal da aplicação.
